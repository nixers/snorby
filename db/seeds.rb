...
SnortSchema.create(:vseq => 107, :ctime => Time.now, :version => "FixNix #{Snorby::VERSION}") if SnortSchema.first.blank?

# Default user setup
User.create(:name => 'Administrator', :email => 'nixers@fixrnix.in', :password => 'fixnix', :password_confirmation => 'fixnix', :admin => true) if User.all.blank?

# Snorby General Settings
Setting.set(:company, 'fixrnix.in') unless Setting.company?
Setting.set(:email, 'nixers@fixrnix.in') unless Setting.email?
